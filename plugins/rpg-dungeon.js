...
*🗑️sampah:* ${sampah  * 4}${potion == 0 ? '' : '\n*🥤Potion:* ' + potion * 4}${makananPet == 0 ? '' : '\n*🍖Makanan Pet* ' + makananPet * 4}${kayu == 0 ? '' : '\n*🪵Kayu:* ' + kayu * 4}${batu == 0 ? '' : '\n*🪨Batu:* ' + batu * 4}${string == 0 ? '' : '\n*🕸️String:* ' + string * 4}${iron == 0 ? '' : '\n*⛓️Iron:* ' + iron * 4}${diamond == 0 ? '' : '\n*💎diamond:* ' + diamond * 4}${common == 0 ? '' : '\n*📦common crate:* ' + common * 4}${uncommon == 0 ? '' : '\n*📦uncommon crate:* ' + uncommon * 4}
          ...
          if ((_H1 || _H2 || _H3 || _H4 || _sd1 || _sd2 || _sd3 || _sd4) < 1) {
            ...
            for (let siapa in player) {
              if ((users[siapa].healt * 1) < 1) HEALTH.push(siapa)
            ...
            let str3 = `${((SDH || SDH.length > 0) || (SDM1L || SDM1L.length > 0)) ? `⚔️Sword ${((SDH || SDH.length > 0 ? sI + ' Hancur, silahkan craft ⚔️Sword kembali dengan mengetik *' + usedPrefix + 'craft sword*' : '') + (SDM1L || SDM1L.length > 0 ? (SDH || SDH.length > 0 ? ', Sedangkan ⚔️Sword ' : '') + sH + ' Hancur, dan Menurun *1* Level' : ''))}` : ''}${HEALTH || HEALTH.length > 0 ? `❤️Nyawa ${H} habis, silahkan isi ❤️Nyawa dengan mengetik ${usedPrefix}heal` : ''}`
            await m.reply(str3, c1, {
              contextInfo: {
                mentionedJid: conn.parseMention(str3)
              }
...
`.trim()
        ...
        if ((users.healt * 1) < 1 || (users.sworddurability * 1) < 1) {
          ...
          let str3 = `${__sword1 ? `⚔️Sword Kamu ${_sword1 ? ` Level nya berkurang 1 karena hancur` : ` Hancur, dan silahkan craft ⚔️Sword kembali dengan mengetik ${usedPrefix}`}craft sword` : ''} ${healt1 ? `${__sword1 ? 'Dan ' : ''}❤️Nyawa Kamu habis, silahkan isi kembali dengan ketik ${usedPrefix}heal` : ''}`
          m.reply(str3, room.player1, {
            contextInfo: {
